...
- 노션에서 가져온 분석서도 편집기에 나옵니다 (`mode: import-once` — 3절 참고). 최초 1회 가져온 뒤엔 여기서 수정하면 됩니다
...
**`mode` — 가져온 뒤 편집기에서 수정할지 여부**

| `mode` | 동작 |
| --- | --- |
| `"sync"` (기본, 생략 시) | 매번 노션에서 다시 가져와 덮어씀. **노션이 원본.** 편집기에서 열리긴 하지만 다음 배포 때 덮어써짐 |
| `"import-once"` | 파일이 없을 때만 생성. 이후엔 **웹 편집기(`/admin/`)에서 다른 글처럼 자유롭게 수정** 가능 |

현재 설정은 `import-once` 입니다. 동작:

- 노션에 새 페이지를 만들고 배포하면 → CI 가 `.md` + 이미지를 만들고 **저장소에 되커밋**함 → 편집기에 나타남
- 그 뒤 편집기에서 수정 → 그 수정이 원본이 됨. CI 는 이미 있는 파일은 건드리지 않음
- 노션 쪽에서 내용을 고쳐도 사이트엔 반영 안 됨 (다시 가져오려면 `src/content/analyses/<slug>.md` 를 지우고 배포)
- `import-once` 에서는 자동 삭제도 꺼집니다 — 문서를 없애려면 편집기에서 삭제
- `overrides` 의 제목/요약/태그는 **최초 생성 시에만** 반영됩니다 (이후엔 편집기에서 직접)
